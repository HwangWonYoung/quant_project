...
def load_required_df():
    ...
    return price, idio_rt


def cal_daily_volatility(stock_rt, month, idio=False):
...
def return_daily_volatility_set(price, idio_rt):
    ...
    daily_vol_1m = cal_daily_volatility(stock_rt, 1)
    daily_vol_3m = cal_daily_volatility(stock_rt, 3)
    daily_vol_6m = cal_daily_volatility(stock_rt, 6)
    daily_vol_12m = cal_daily_volatility(stock_rt, 12)
    daily_vol_24m = cal_daily_volatility(stock_rt, 24)

    idio_daily_vol_1m = cal_daily_volatility(idio_rt, 1, idio=True)
    idio_daily_vol_3m = cal_daily_volatility(idio_rt, 3, idio=True)
    idio_daily_vol_6m = cal_daily_volatility(idio_rt, 6, idio=True)
    idio_daily_vol_12m = cal_daily_volatility(idio_rt, 12, idio=True)
    idio_daily_vol_24m = cal_daily_volatility(idio_rt, 24, idio=True)
...
def update_daily_vol_table():
    ...
    price, idio_rt = load_required_df()
